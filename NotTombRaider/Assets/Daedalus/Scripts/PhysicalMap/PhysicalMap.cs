...
abstract public class PhysicalMap<T> : PhysicalMap where T : PhysicalMapBehaviour
{
    ...
    override protected void FinalizeGeneration()
    {
        ...
        if (behaviour.createPlayerThief) PlacePlayerThief();
        if (behaviour.createPlayerFighter) PlacePlayerFighter();
    ...
    private void PlacePlayerThief()
    {
        if (behaviour.createEntranceAndExit && behaviour.thiefPrefab != null)
        {
            GameObject playerGo = GameObject.Instantiate(behaviour.thiefPrefab) as GameObject;
            Vector3 position = this.GetStartPosition();
            position.x = position.x - 1;

            playerGo.transform.position = position;// + Vector3.up*2;
            playerGo.transform.parent = this.dynamicMapTr;
            playerGo.name = "Thief";
        }
    }

    private void PlacePlayerFighter()
    {
        if (behaviour.createEntranceAndExit && behaviour.fighterPrefab != null)
        {
            GameObject playerGo = GameObject.Instantiate(behaviour.fighterPrefab) as GameObject;
            Vector3 position = this.GetStartPosition();
            position.x = position.x + 1;
            playerGo.transform.position = position;// + Vector3.up*2;
            playerGo.transform.parent = this.dynamicMapTr;
            playerGo.name = "Fighter";
        }
    }
...
abstract public class PhysicalMap : ScriptableObject
{
    ...
    public void AddToMapGameObject(VirtualCell.CellType cell_type, GameObject go, bool dynamic, int zone = 0)
    {   // TODO: extend zones, for now just used for the storeys
